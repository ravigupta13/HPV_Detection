...
from models.resnet_custom_dep import resnet34_baseline
...
for idx,(s_name, s_path) in enumerate(tqdm(zip(slide_names, slide_paths))):
	if idx == 108 or idx == 322 or os.path.exists(os.path.join(args.result_dir, s_name + '_all.png')):
		continue
	...
	for i in range(len(coords)):
		# print(some_value.shape)
		some_value= test_img[coords[i,0]//down:coords[i,0]//down +args.patch_size//down, 
	...
	# Image.fromarray((attention_img.transpose(1,0,2)*255).astype(np.uint8)).show()
	Image.fromarray((final_image.transpose(1,0,2)).astype(np.uint8)).save(os.path.join(args.result_dir, s_name + '_all.png'))
	continue
	# Image.fromarray((background.transpose(1,0,2)).astype(np.uint8)).show()
	# exit()
	# Image.fromarray(background).show()
	for per in [0.75,0.85,0.95,0.99]:
		img = np.ones((m//down, n//down,3), 'uint8') *255	
		img2 = np.ones((m//down, n//down,3), 'uint8') *255	
		thres = np.quantile(A_soft,per)
		test_img = np.zeros((m//down, n//down), 'uint8') *255
		new_test_img = np.zeros((m//down, n//down,3), 'uint8') *255
		slide_img = np.zeros((m//down, n//down,3), 'uint8') *255
		for i in range(len(coords)):
			test_img[coords[i,0]//down:coords[i,0]//down +args.patch_size//down, 
				coords[i,1]//down:coords[i,1]//down + args.patch_size//down] = np.power(A_soft_log_norm[i],0.7)*255
			new_test_img[coords[i,0]//down:coords[i,0]//down +args.patch_size//down, 
				coords[i,1]//down:coords[i,1]//down + args.patch_size//down,:] = (cmap(test_img[coords[i,0]//down:coords[i,0]//down +args.patch_size//down, 
				coords[i,1]//down:coords[i,1]//down + args.patch_size//down]))[:,:,:3]
			slide_img[coords[i,0]//down:coords[i,0]//down +args.patch_size//down, 
				coords[i,1]//down:coords[i,1]//down + args.patch_size//down,:] = np.array(slide.read_region((coords[i,1],coords[i,0]),0,(args.patch_size,args.patch_size)).convert('RGB').resize((args.patch_size//down,args.patch_size//down)))
			
			if(A_soft[i]>=thres):
				img[coords[i,0]//down:coords[i,0]//down +args.patch_size//down, 
				coords[i,1]//down:coords[i,1]//down + args.patch_size//down,1:] = A_soft_log_norm[i]*255
			# 	img[coords[i,0]//down:coords[i,0]//down +args.patch_size//down, 
			# coords[i,1]//down:coords[i,1]//down + args.patch_size//down,0] = thumb[coords[i,0]//down:coords[i,0]//down +args.patch_size//down, 
			# coords[i,1]//down:coords[i,1]//down + args.patch_size//down]
			else:
				img[coords[i,0]//down:coords[i,0]//down +args.patch_size//down, 
				coords[i,1]//down:coords[i,1]//down + args.patch_size//down,:2] = A_soft_log_norm[i]*255 
				# img[coords[i,0]//down:coords[i,0]//down +args.patch_size//down, 
				# coords[i,1]//down:coords[i,1]//down + args.patch_size//down,2] = thumb[coords[i,0]//down:coords[i,0]//down +args.patch_size//down, 
				# coords[i,1]//down:coords[i,1]//down + args.patch_size//down]

		
		Image.fromarray((slide_img.transpose(1,0,2)*255).astype(np.uint8)).show()
		Image.fromarray((new_test_img.transpose(1,0,2)*255).astype(np.uint8)).show()
		exit()
		img = Image.fromarray(img.transpose(1,0,2),'RGB')
		path = os.path.join(args.result_dir, s_name + '_' + str(per)+'_.png')
		# img.show()
		# break
		# img[:,:,1] = 
		# plt.imshow(img2[:,:,1].T, cmap='jet')
		# plt.colorbar()
		# plt.show()
		img.save(path)
		# exit()
	...
	print("Saved {}/{} Heatmaps".format(idx+1, len(slide_names)))
	# plt.imshow(img)
	# plt.show()
